import {FormClientes, FormDirecciones, FormDireccionesEdit, FormFacturas, FormFacturasEdit} from "./formClientes";
...
export const ModalFacturasEdit = ({ hdlData, hdlSubmit, tglModal, isLoading, data, isLoadingData }) => {
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-opa-50">
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-sm border border-gray-300">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">
        ...
        <div>
          <FormFacturasEdit fun={hdlData} data={data} isLoadingData={isLoadingData}/>
        </div>
        <div className="mt-6 flex justify-end space-x-4">
          <button
            onClick={tglModal}
          ...
          <button
            ...
            className="px-6 py-2 text-white bg-yellow-600 rounded-lg hover:bg-yellow-600 transition duration-300 ease-in-out transform hover:scale-105"
          >
            {isLoading ? 'Modificando...' : isLoadingData ? 'Cargando...' : 'Modificar'}
          </button>
        </div>
      </div>
    </div>
